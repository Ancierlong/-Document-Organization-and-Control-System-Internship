...
$sortBy = isset($_GET['sortBy']) ? $_GET['sortBy'] : 'ojt_full_name';
...
$allowedSortColumns = ['ojt_full_name', 'ojt_email', 'ojt_telno', 'ojt_address', 'ojt_description', 'ojt_company', 'file_name', 'uploaded_at']; // Added file_name and uploaded_at if sortable
if (!in_array($sortBy, $allowedSortColumns)) {
    $sortBy = 'ojt_full_name'; // Default if invalid
}
...
$totalSql = "SELECT COUNT(*) AS total FROM ojtrecords
             WHERE (ojt_full_name LIKE ? OR ojt_company LIKE ?)
             AND archive = 0";
...
$sql = "SELECT id, ojt_full_name, ojt_email, ojt_telno, ojt_address, ojt_description, ojt_company,
               file_name, uploaded_at
        FROM ojtrecords
        WHERE (ojt_full_name LIKE ? OR
               ojt_email LIKE ? OR
               ojt_telno LIKE ? OR
               ojt_address LIKE ? OR
               ojt_description LIKE ? OR
               ojt_company LIKE ? OR
               file_name LIKE ? OR
               uploaded_at LIKE ?) -- Added file_name and uploaded_at to search fields
        AND archive = 0
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        // Corrected file path assumption to file_ojt_records
        $filePath = "../file_ojt_records/" . urlencode($row["file_name"]);

        // Check if the file exists using the absolute server path for robustness
        $uploadDir = realpath(__DIR__ . '/../file_ojt_records/');
        ...
        $ojtId = htmlspecialchars($row['id']); // Get the ID for buttons


        $tableHtml .= "<tr>";
        $tableHtml .= "<td>" . htmlspecialchars($row["ojt_full_name"]) . "</td>";
        $tableHtml .= "<td>" . htmlspecialchars($row["ojt_email"]) . "</td>";
        $tableHtml .= "<td>" . htmlspecialchars($row["ojt_telno"]) . "</td>";
        $tableHtml .= "<td>" . htmlspecialchars($row["ojt_address"]) . "</td>";
        $tableHtml .= "<td>" . htmlspecialchars($row["ojt_description"]) . "</td>";
        $tableHtml .= "<td>" . htmlspecialchars($row["ojt_company"]) . "</td>";
        ...
        $tableHtml .= "<form action='edit_ojt_records.php' method='post' style='display:inline-block;'>";
        $tableHtml .= "<input type='hidden' name='id' value='{$ojtId}'>";
        ...
        if ($role === 'Admin') { // Only show the button if the role is Admin
            $tableHtml .= "<form action='archive_ojt_records.php' method='POST' onsubmit='return confirmDelete()'>";
            $tableHtml .= "<input type='hidden' name='id' value='{$ojtId}'>";
            // Include uploaded_at if archive_ojt_records.php requires it
