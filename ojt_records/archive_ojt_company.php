...
$sql = "UPDATE company_linkages set archive = 1 where id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);

if ($stmt->execute()) {
    // Log the archive  
    $logSql = "INSERT INTO logs_company_linkages (type, modified_item, user) VALUES (?, ?, ?)";
    ...
    header("Location: view_ojt_companies.php");
