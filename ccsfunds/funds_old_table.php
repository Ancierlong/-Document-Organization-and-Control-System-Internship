<?php
 ...
 function updateBalanceAndLog($conn, $concept_id, $type, $amount, $userid, $concepts) {
     ...
     try {
         ...
         $stmt_get_balance = $conn->prepare("SELECT balance FROM funds WHERE id = 1 FOR UPDATE");
          ...
          // REMOVE THIS LINE: $stmt_get_balance->close();
         ...
         // REMOVE THIS LINE: $stmt_update->close();
         ...
         // Use NULL for concept_id if it's 0 (General Fund), otherwise cast to int
         $concept_id_for_db = ($concept_id == 0) ? null : (int)$concept_id;
         ...
         // A common approach if concept_id can be NULL:
          if ($concept_id_for_db === null) {
               // Adjust query slightly if needed or ensure column allows NULL
               // Re-prepare with NULL directly might be needed depending on driver/strictness
               // For this example, let's assume binding NULL to 'i' works or the column handles it.
               // IMPORTANT: Verify your `logs_funds` table schema allows NULL for `concept_id`.
               $log_query_null = "INSERT INTO logs_funds (concept_id, transaction_type, amount, transaction_date, user_id, previous_balance, new_balance) VALUES (NULL, ?, ?, ?, ?, ?, ?)";
               $stmt_log = $conn->prepare($log_query_null);
               if (!$stmt_log) { throw new Exception("Error preparing NULL log statement: " . $conn->error); }
               $stmt_log->bind_param("ssdsdd", $type, $amount, $datetime, $userid, $current_balance, $new_balance); // Note: userid might be int, adjust 's' if so
          } else {
              // If concept_id is not NULL, bind as integer
         ...
         // REMOVE THIS LINE: $stmt_log->close();
 ...
 $log_query = "SELECT
                      ...
                      LEFT JOIN conceptpapers c ON lf.concept_id = c.id -- Use LEFT JOIN to include General Fund entries (where concept_id is NULL or 0)
                 ...
                 <tbody>
                     <?php foreach ($logs as $log): ?>
                         <tr>
                             ...
                             <td>
                                  <?php echo htmlspecialchars($log['concept_title'] ?: 'General Fund'); // Display 'General Fund' if title is null/empty ?>
                             </td>
                             <td>
                                  <?php echo $log['concept_date'] ? htmlspecialchars(date('Y-m-d', strtotime($log['concept_date']))) : 'N/A'; // Display N/A if no date ?>
                             </td>
