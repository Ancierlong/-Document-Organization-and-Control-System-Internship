...
<head>
    ...
    <style>
        ...
        /* There are 8 columns: Title, Description, Academic Year, Date, File, Posted By, Edit, Archive */
        .searchemp-container th:nth-child(1), .searchemp-container td:nth-child(1) { width: 15%; } /* Title */
        .searchemp-container th:nth-child(2), .searchemp-container td:nth-child(2) { width: 20%; } /* Description */
        .searchemp-container th:nth-child(3), .searchemp-container td:nth-child(3) { width: 10%; } /* Academic Year */
        .searchemp-container th:nth-child(4), .searchemp-container td:nth-child(4) { width: 10%; } /* Date */
        .searchemp-container th:nth-child(5), .searchemp-container td:nth-child(5) { width: 15%; } /* File */
        .searchemp-container th:nth-child(6), .searchemp-container td:nth-child(6) { width: 10%; } /* Posted By */
...
<body>
    <div class="searchemp-container" data-user-role="<?php echo htmlspecialchars($user_role); ?>">
        ...
        <div class="tophead2">
            <h4>CCS-DOCS</h4>
            <h2>Activity Reports List</h2>
            <h3>Search: <input type="text" class="search" id="search"
                    placeholder="Search by Title or Date..."></h3>
            <a href="add_activity_reports.php" class="button">Add Activity Report</a>
        </div>
        ...
        <table>
            <thead>
                <tr>
                    <th onclick="sortTable('activity_title')">Title</th>
                    <th onclick="sortTable('activity_description')">Description</th>
                    <th onclick="sortTable('academic_year')">Academic Year</th>
                    <th onclick="sortTable('activity_date')">Date<br>(yyyy/mm/dd)</th>
                    <th onclick="sortTable('file_name')">File</th>
                    <th onclick="sortTable('posted_by')">Posted By</th>
                    <th>Edit</th>
                    <th>Archive</th>
                </tr>
            </thead>
    ...
    <script>
        let currentPage = 1;
        let rowsPerPage = 10;
        let currentSortBy = 'activity_date';
        ...
        function loadTable() {
            ...
            let url = `retrieve_activity_reports.php?search=${encodeURIComponent(currentSearchValue)}&sortBy=${encodeURIComponent(currentSortBy)}&sortOrder=${encodeURIComponent(currentSortOrder)}&page=${currentPage}&rowsPerPage=${rowsPerPage}`;
                ...
                if (xhr.status == 200) {
                     ...
                     if (response) {
                        ...
                        console.error("Error: No JSON response received.");
                         ...
                         document.getElementById("tableData").innerHTML = "<tr><td colspan='8'>Error loading data. Empty response.</td></tr>";
            ...
            xhr.onerror = function () {
                console.error("Network error loading table.");
                 // Use max possible colspan for error message (8 columns)
                document.getElementById("tableData").innerHTML = "<tr><td colspan='8'>Network error loading data.</td></tr>";
                document.getElementById("paginationControls").innerHTML = "";
            };
            xhr.send();
        }
