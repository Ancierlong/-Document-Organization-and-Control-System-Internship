...
function generateLogTableHTML($conn, $itemsPerPage, $currentPage, $searchTerm, $sortBy = 'l.date', $sortDirection = 'DESC') {
    ...
    function getLogData($conn, $limit, $offset, $searchTerm = '', $sortBy = 'l.date', $sortDirection = 'DESC') {
        $sql = "
            SELECT
                l.type AS log_type,
                cp.concept_title AS modified_item_title, -- Corrected table alias to cp
                u.username AS modified_by_user,
                l.date AS log_date
            FROM
                logs_concept_papers l
            JOIN
                conceptpapers cp ON l.modify_id = cp.uploaded_at -- Corrected table alias to cp
            JOIN
        ...
        if (!empty($searchTerm)) {
            ...
            $whereClauses[] = "(l.type LIKE '%$escapedSearchTerm%'
                                OR cp.concept_title LIKE '%$escapedSearchTerm%' -- Corrected table alias to cp
        ...
        // Added 'cp.concept_title' to valid sort columns
        $validSortColumns = ['l.type', 'cp.concept_title', 'u.username', 'l.date'];
    ...
    function getTotalLogCount($conn, $searchTerm = '') {
        $sql = "SELECT COUNT(*) AS total FROM logs_concept_papers l
                     JOIN conceptpapers cp ON l.modify_id = cp.uploaded_at -- Corrected table alias to cp
    ...
    if (!empty($logData) || $totalLogs > 0) { // Show table header even if no data if total > 0 (filtered)
        ...
        $html .= "<thead><tr>
                    ...
                    <th data-sort-by='cp.concept_title' class='sortable'>Concept Title</th> <th data-sort-by='u.username' class='sortable'>User</th>
...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    $validSortColumns = ['l.type', 'cp.concept_title', 'u.username', 'l.date']; // Corrected table alias
    $validSortDirections = ['ASC', 'DESC'];
...
<head>
    ...
    <title>Logs Concept Papers</title>
    <style>
        body {
