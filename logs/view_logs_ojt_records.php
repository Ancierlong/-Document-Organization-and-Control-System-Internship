...
function generateLogTableHTML($conn, $itemsPerPage, $currentPage, $searchTerm, $sortBy = 'l.date', $sortDirection = 'DESC') {
    ...
    function getLogData($conn, $limit, $offset, $searchTerm = '', $sortBy = 'l.date', $sortDirection = 'DESC') {
        $sql = "
            SELECT
                l.type AS log_type,
                ojt.ojt_full_name AS modified_item_title, -- Corrected table alias to ojt
                u.username AS modified_by_user,
                l.date AS log_date
            FROM
                logs_ojt_records l
            JOIN
                ojtrecords ojt ON l.modified_item = ojt.uploaded_at -- Corrected table alias to ojt
            JOIN
        ...
        if (!empty($searchTerm)) {
            ...
            $whereClauses[] = "(l.type LIKE '%$escapedSearchTerm%'
                                OR ojt.ojt_full_name LIKE '%$escapedSearchTerm%' -- Corrected table alias to ojt
        ...
        // Added 'ojt.ojt_full_name' to valid sort columns
        $validSortColumns = ['l.type', 'ojt.ojt_full_name', 'u.username', 'l.date'];
    ...
    function getTotalLogCount($conn, $searchTerm = '') {
        $sql = "SELECT COUNT(*) AS total FROM logs_ojt_records l
                     JOIN ojtrecords ojt ON l.modified_item = ojt.uploaded_at -- Corrected table alias to ojt
    ...
    if (!empty($logData) || $totalLogs > 0) { // Show table header even if no data if total > 0 (filtered)
        ...
        $html .= "<thead><tr>
                    ...
                    <th data-sort-by='ojt.ojt_full_name' class='sortable'>OJT Name</th> <th data-sort-by='u.username' class='sortable'>User</th>
...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    $validSortColumns = ['l.type', 'ojt.ojt_full_name', 'u.username', 'l.date']; // Corrected table alias
    $validSortDirections = ['ASC', 'DESC'];
