...
function generateLogTableHTML($conn, $itemsPerPage, $currentPage, $searchTerm, $sortBy = 'l.date', $sortDirection = 'DESC') {
    ...
    function getLogData($conn, $limit, $offset, $searchTerm = '', $sortBy = 'l.date', $sortDirection = 'DESC') {
        // Modified SQL query: Removed JOIN to a 'modified item' table
        $sql = "
            SELECT
                l.action AS log_type,
                u.username AS modified_by_user, -- Renamed for consistency in output
                l.date AS log_date -- Renamed for consistency in output
            FROM
                user_logs l
            JOIN
        ...
        if (!empty($searchTerm)) {
            ...
            // Adjusted search fields to match selected columns
            $whereClauses[] = "(l.action LIKE '%$escapedSearchTerm%'
        ...
        // Updated valid sort columns
        $validSortColumns = ['l.action', 'u.username', 'l.date'];
    ...
    function getTotalLogCount($conn, $searchTerm = '') {
        // Modified SQL query: Removed JOIN to a 'modified item' table
        $sql = "SELECT COUNT(*) AS total FROM user_logs l
                     JOIN users u ON l.user = u.id";
        if (!empty($searchTerm)) {
            // Using LIKE with wildcards requires manual escaping if not using prepared statements
            $escapedSearchTerm = $conn->real_escape_string($searchTerm);
             // Adjusted search fields to match selected columns
            $sql .= " WHERE l.action LIKE '%$escapedSearchTerm%'
                                OR u.username LIKE '%$escapedSearchTerm%'
                                OR l.date LIKE '%$escapedSearchTerm%'";
        }
        $result = $conn->query($sql);
    ...
    $html = '';
     ...
     // Show table header even if no data if total > 0 (filtered)
    if (!empty($logData) || $totalLogs > 0) {
        $html .= "<table id='logTable'>";
        // Updated table headers and data-sort-by attributes
        $html .= "<thead><tr>
                    <th data-sort-by='l.action' class='sortable'>Action</th>
        ...
        if (!empty($logData)) {
            foreach ($logData as $log) {
                $html .= "<tr>";
                 // Adjusted column names based on SELECT query aliases
                $html .= "<td>" . htmlspecialchars($log['log_type']) . "</td>";
                $html .= "<td>" . htmlspecialchars($log['modified_by_user']) . "</td>"; // Using alias modified_by_user
                $html .= "<td>" . htmlspecialchars($log['log_date']) . "</td>"; // Using alias log_date
                $html .= "</tr>";
            }
        } else {
             // Corrected colspan based on the new number of columns
             $html .= "<tr><td colspan='3'>No matching log data found.</td></tr>";
        }
...
if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    // Input validation for safety - Updated valid sort columns
...
// Input validation for initial GET load - Updated valid sort columns
$currentPage = max(1, $currentPage); // Page number cannot be less than 1
$validSortColumns = ['l.action', 'u.username', 'l.date'];
$validSortDirections = ['ASC', 'DESC'];
$sortBy = in_array($sortBy, $validSortColumns) ? $sortBy : 'l.date';
