...
function generateLogTableHTML($conn, $itemsPerPage, $currentPage, $searchTerm, $sortBy = 'l.date', $sortDirection = 'DESC') {
    ...
    function getLogData($conn, $limit, $offset, $searchTerm = '', $sortBy = 'l.date', $sortDirection = 'DESC') {
        $sql = "
            SELECT
                l.type AS log_type,
                ar.activity_title AS modified_item_title, -- Corrected table alias to ar
                u.username AS modified_by_user,
                l.date AS log_date
            FROM
                logs_activity_reports l
            JOIN
                activityreports ar ON l.modified_item = ar.uploaded_at -- Corrected table alias to ar
            JOIN
        ...
        if (!empty($searchTerm)) {
            ...
            $whereClauses[] = "(l.type LIKE '%$escapedSearchTerm%'
                                OR ar.activity_title LIKE '%$escapedSearchTerm%' -- Corrected table alias to ar
        ...
        $validSortColumns = ['l.type', 'ar.activity_title', 'u.username', 'l.date']; // Corrected table alias to ar
    ...
    function getTotalLogCount($conn, $searchTerm = '') {
        $sql = "SELECT COUNT(*) AS total FROM logs_activity_reports l
                     JOIN activityreports ar ON l.modified_item = ar.uploaded_at -- Corrected table alias to ar
    ...
    if (!empty($logData) || $totalLogs > 0) { // Show table header even if no data if total > 0 (filtered)
        ...
        $html .= "<thead><tr>
                    ...
                    <th data-sort-by='ar.activity_title' class='sortable'>Activity Title</th> <th data-sort-by='u.username' class='sortable'>User</th>
...
<head>
    ...
    <title>Logs Activity Reports</title>
    <style>
        body {
