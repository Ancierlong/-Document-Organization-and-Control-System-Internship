...
$sql = "UPDATE thesiscapstoneprojects set archive = 1 where id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $id);

if ($stmt->execute()) {
    // Log the archive  
    $logSql = "INSERT INTO logs_thesis_capstone_projects (type, modified_item, user) VALUES (?, ?, ?)";
    ...
    header("Location: view_capstone_thesis_test.php");
